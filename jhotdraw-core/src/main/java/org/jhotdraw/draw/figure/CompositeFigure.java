 ...
 *
 * <p>A composite figure can be laid out using a {@link Layouter}.
 *
 * <p>{@code CompositeFigure} listens to {@code requestRemove} events sent by its child figures, and
 * then removes the child figure which sent the event.
 *
 ...
 * observes area invalidations of its drawing object.<br>
 ...
 * Subject: {@link CompositeFigure}; Observer: {@link
 * org.jhotdraw.draw.event.CompositeFigureListener}; Event: {@link
...
public interface CompositeFigure extends Figure {
   ...
   *
   * <p>This method can be used to reinsert a child figure which has been temporarily removed from
   * this CompositeFigure (for example to reorder the sequence of the children) and to efficiently
   * build a drawing from an {@link org.jhotdraw.draw.io.InputFormat}.
   *
   * <p>This is a convenience method for calling {@code basicAdd(getChildCount(), child);}.
   */
   ...
   *
   * <p>This is a convenience method for calling {@code removeChild(getChildren().indexOf(child));}
   *
   * <p>This method calls {@code figureRemoved} on all registered {@code CompositeFigureListener}'s.
   */
   ...
   *
   * <p>Calls {@code figureRemoved} on all registered {@code CompositeFigureListener}'s.
   */
   ...
   *
   * <p>This method can be used to temporarily remove a child from this CompositeFigure (for example
   * to reorder the sequence of the children).
   *
   * <p>This is a convenience method for calling {@code basicRemove(indexOf(child));}.
   *
   * <p>Returns the index of the removed figure. Returns -1 if the figure was not a child of this
   * CompositeFigure.
   */
   ...
   * to reorder the sequence of the children).
   *
   * <p>Returns the removed child figure.
   */
   ...
   * to reorder the sequence of the children).
   *
   * <p>This is a convenience method for {@code while(getChildCount() > 0) basicRemoveChild(0); }
   */
   ...
   *
   * <p>This is a convenience method for calling {@code getChildren().contains(f);}.
   */
   ...
   * each other.
   *
   * <p>This method first calls layout() on all child figures which implement the CompositeFigure
   * interface. Then the children are laid out.
   *
   ...
   * Layouter accesses the child components of this figure and arranges their graphical
   * presentation. It is a good idea to put the Layouter in the protected initialize() method so it
