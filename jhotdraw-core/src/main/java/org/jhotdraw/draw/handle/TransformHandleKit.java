...
public class TransformHandleKit {
    ...
    public void draw(Graphics2D g) {
      if (getEditor().getTool().supportsHandleInteraction()) {
        ...
        drawDiamond(
            g,
            getEditor()
                .getHandleAttribute(HandleAttributeKeys.TRANSFORM_HANDLE_FILL_COLOR_DISABLED),
            getEditor()
                .getHandleAttribute(HandleAttributeKeys.TRANSFORM_HANDLE_STROKE_COLOR_DISABLED));
      }
    }
  ...
  private static class EastHandle extends TransformHandle {
    ...
    protected void trackStepNormalized(Point2D.Double p) {
      Rectangle2D.Double r = getTransformedBounds();
      transform(
          new Point2D.Double(r.x, r.y), new Point2D.Double(Math.max(r.x + 1, p.x), r.y + r.height));
    }
  ...
  private static class SouthHandle extends TransformHandle {
    ...
    protected void trackStepNormalized(Point2D.Double p) {
      Rectangle2D.Double r = getTransformedBounds();
      transform(
          new Point2D.Double(r.x, r.y), new Point2D.Double(r.x + r.width, Math.max(r.y + 1, p.y)));
    }
