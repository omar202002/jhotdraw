...
public interface Application {
   ...
   *
   * <p>This method implements behavior for the following features: <em>Launch application</em>,
   * <em>Open last URI on launch</em>. See {@link org.jhotdraw.api.app}.
   *
   ...
   * <p>This method must be called from the main thread of the application.
   *
   * <p>This method implements behavior for the following feature: <em>Open URIs from command line
   * on launch</em>. See {@link org.jhotdraw.api.app}.
   *
   * @param args The arguments of the main method
   */
   ...
   *
   * <p>By convention this method is only called after {@link #configure} has been called.
   *
   * <p>Typically in this method, the Application sets property name identifier for
   * application-specific properties in {@code ResourceBundleUtil}. For example with {@code
   ...
   *
   * <p>This method creates a view for each supplied URI, and adds it to the application. If no URI
   * has been supplied, this method may open an empty view.
   *
   * <p>By convention this method is only called after {@link #init} has been called.
   *
   * <p>This method implements behavior for the following feature: <em>Open URI on launch</em>. See
   * {@link org.jhotdraw.api.app}.
   *
   * @param uris Upon launch, the application may be requested to open views for a given list of
   *     URI's.
   */
   ...
   * <p>By convention this method is only called after {@link #init} has been called.
   *
   * <p>This method must be called from AWT Event Dispatcher Thread.
   */
  public void stop();
  ...
  /**
   * Creates a new view for this application and initializes it, by calling {@link View#init}. The
   * view has not been added to the application yet. To make the view usable with this application,
   * call {@link #add(View)}. To make it visible, first call {@code add(View)}, then {@link
   * #show(View)}.
   *
  ...
  /**
   * Adds a view to this application. Fires a "documentCount" property change event. Invokes method
   * setApplication(this) on the view object.
   *
   * @param v the view
   */
  ...
  /**
   * Removes a view from this application and removes it from the users view. Fires a
   * "documentCount" property change event. Invokes method setApplication(null) on the view object.
   *
   * @param v the view
   */
  ...
  /**
   * Returns the active view. This is used for OSXApplication and MDIApplication which share actions
   * among multiple View instances. Active view may be become null, if the application has no view.
   *
   ...
   *
   * <p>The enabled state is used to prevent parallel invocation of actions on the application. If
   * an action consists of a sequential part and a concurrent part, it must disable the application
   * only for the sequential part.
   *
   * <p>Actions that act on the application must check in their actionPerformed method whether the
   * application is enabled. If the application is disabled, they must do nothing. If the
   * application is enabled, they must disable the application, perform the action and then enable
  ...
  /**
   * Returns the application component. This may return null, if the application is not represented
   * by a component of its own on the user interface.
   *
   * @return the value
   */
   ...
   * @param view The View to which this window is associated, or null if the window is associated to
   *     the application.
   */
  ...
  /**
   * Returns the recently opened URIs. By convention, this is an unmodifiable list. The first item
   * in the list is the most recently opened URI.
   *
   * <p>The most recent URI is used by the <em>Open last URI on launch</em> feature. See {@link
   ...
   *
   * <p>This fires a property change event for the property "recentURIs".
   *
   * <p>The recent URIs are persisted (for example using the Java Preferences API) so that they are
   * available on the next launch of the application.
   *
   * <p>The most recent URI is used by the <em>Open last URI on launch</em> feature. See {@link
   * org.jhotdraw.api.app}.
   *
  ...
  /**
   * Clears the list of recent URIs. This fires a property change event for the property
   * "recentURIs".
   *
   * <p>The recent URIs are persisted (for example using the Java Preferences API) so that they are
   * available on the next launch of the application.
   ...
   *
   * <p>The list of views is used by the <em>Allow multiple views per URI</em> feature. See {@link
   * org.jhotdraw.api.app}.
   *
   * @return the views
   */
