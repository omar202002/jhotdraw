 ...
 * application, when the {@code ApplicationModel} provides a {@code LoadFileAction} or a {@code
 * OpenFileAction}.
 *
  ...
  private PropertyChangeListener createApplicationListener() {
    return new PropertyChangeListener() {
      ...
      public void propertyChange(PropertyChangeEvent evt) {
        if ((evt.getPropertyName() == null && Application.RECENT_URIS_PROPERTY == null)
            || (evt.getPropertyName() != null
                && evt.getPropertyName()
                    .equals(Application.RECENT_URIS_PROPERTY))) { // Strings get interned
          updateEnabled();
        }
      }
    };
