...
public interface URIChooser {
  ...
  /**
   * Returns the type of this dialog. The default is {@code URIChooser.OPEN_DIALOG}.
   *
  ...
  /**
   * Sets the type of this dialog. Use {@code OPEN_DIALOG} when you want to bring up a chooser that
   * the user can use to open an URI. Likewise, use {@code SAVE_DIALOG} for letting the user choose
   * an URI for saving. Use {@code CUSTOM_DIALOG} when you want to use the chooser in a context
   * other than "Open" or "Save". For instance, you might want to bring up a chooser that allows the
   * user to choose an URI to execute. Note that you normally would not need to set the {@code
   * URIChooser} to use {@code CUSTOM_DIALOG} since a call to {@code setApproveButtonText} does this
   * for you. The default dialog type is {@code URIChooser.OPEN_DIALOG}.
   *
   ...
   *     </ul>
   *
   * @exception IllegalArgumentException if {@code dialogType} is not legal
   * @see #getDialogType
  ...
  /**
   * Returns the text used in the {@code ApproveButton} . If {@code null} , the UI object will
   * determine the button's text.
   *
   * <p>Typically, this would be "Open" or "Save".
   *
   ...
   *
   * <p>Typically, this would return {@code this}.
   *
  ...
  /**
   * Sets the string that goes in the {@code URIChooser} window's title bar.
   *
  ...
  // **************************************
  /**
   * Pops up an "Open" chooser dialog. Note that the text that appears in the approve button is
   * determined by the L&amp;F.
   *
   * @param parent the parent component of the dialog, can be {@code null}; see {@code showDialog}
   *     for details
   * @return the return state of the file chooser on popdown:
   *     <ul>
   *       <li>URIChooser.CANCEL_OPTION
   *       <li>URIChooser.APPROVE_OPTION
   *       <li>URIChooser.ERROR_OPTION if an error occurs or the dialog is dismissed
   *     </ul>
   *
   ...
   *      "depends" means.-->
   *
   * <p>The {@code parent} argument determines two things: the frame on which the open dialog
   * depends and the component whose position the look and feel should consider when placing the
   * dialog. If the parent is a {@code Frame</code> object (such as a <code>JFrame} ) then the
   ...
   * example, centered over the component). If the parent is {@code null} , then the dialog depends
   * on no visible window, and it's placed in a look-and-feel-dependent position such as the center
   * of the screen.
   *
