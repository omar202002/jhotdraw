...
public class PaletteUtilities extends BasicGraphicsUtils {
  ...
  /**
   * Draw a string with the graphics {@code g} at location ({@code x}, {@code y}) just like {@code
   * g.drawString} would. The character at index {@code underlinedIndex} in text will be underlined.
   * If {@code index} is beyond the bounds of {@code text} (including &lt; 0), nothing will be
   * underlined.
   *
   ...
   *
   * <p>Callers should pass in a non-null JComponent, the exception to this is if a JComponent is
   * not readily available at the time of painting.
   *
   * <p>This does not necessarily return the FontMetrics from the Graphics.
   *
  ...
  /**
   * Returns the FontMetrics for the specified Font. This method is used when a Graphics is
   * available, typically when painting. If a Graphics is not available the JComponent method of the
   * same name should be used.
   *
   * <p>Callers should pass in a non-null JComonent, the exception to this is if a JComponent is not
