...
public abstract class AbstractApplication extends AbstractBean implements Application {
   ...
   *
   * @param args This implementation supports the command-line parameter "-open" which can be
  ...
  protected void removeTrailingSeparators(JMenu m) {
    JPopupMenu pm = m.getPopupMenu();
    for (int i = pm.getComponentCount() - 1;
        i > 0 && (pm.getComponent(i) instanceof JSeparator);
        i--) {
      pm.remove(i);
    }
  }
  ...
  private class OpenRecentMenuHandler implements PropertyChangeListener, Disposable {
    ...
    public void propertyChange(PropertyChangeEvent evt) {
      ...
      if ((name == null && RECENT_URIS_PROPERTY == null)
          || (name != null && name.equals(RECENT_URIS_PROPERTY))) {
        updateOpenRecentMenu();
      }
    }
