...
public class BoundedRangeInputStream extends FilterInputStream implements BoundedRangeModel {
   ...
   *
   * <p>Returns the model's current value. Note that the upper limit on the model's value is {@code
   * maximum - extent} and the lower limit is {@code minimum}.
   *
   ...
   * greater than {@code maximum} then it's set to {@code maximum}, and if it's greater than {@code
   * value+extent} then it's set to {@code value+extent}.
   *
