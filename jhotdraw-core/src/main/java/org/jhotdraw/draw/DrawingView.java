 ...
 *
 * <p>A drawing view can hold only one drawing at a time, but a drawing can be in multiple drawing
 * views at the same time.
 *
 * <p>{@code DrawingView} can paint the drawing with a scale factor. It supports conversion between
 * view coordinates and drawing coordinates.
 *
 * <p>{@code DrawingView} maintains a selection of the {@link Figure}s contained in the drawing.
 * When a figure is selected, the drawing view request {@link Handle}s from the figure and draws
 * them on top of the drawing.
 *
 * <p>To support editing, a drawing view needs to be added to a {@link DrawingEditor}. The current
 * {@link org.jhotdraw.draw.tool.Tool} of the drawing editor receives mouse and key events from all
 * drawing views of the drawing editor. When added to an editor, the drawing view paints the current
 * {@link org.jhotdraw.draw.tool.Tool} on top of the drawing. The selected figures and handles can
 * be the targets of the current tool of the drawing editor.
 *
 * <p>Editing operations can be constrained, for example by a grid. The constraints are defined by a
 * {@link Constrainer} object. Handles and tools should take the constrainer of the drawing view
 * into account when editing a figure.
 *
 * <p>The painting process of {@code DrawingView} view involves the following steps:
 *
 * <ol>
 *   <li>Paint the background of the drawing view.
 *   <li>Invoke {@link Drawing#drawCanvas}.
 *   <li>Invoke {@link Constrainer#draw} if a constrainer is set.
 *   <li>Invoke {@link Drawing#draw}.
 *   <li>Invoke {@link org.jhotdraw.draw.handle.Handle#draw} on the handles of selected figures.
 *   <li>Invoke {@link org.jhotdraw.draw.tool.Tool#draw} if the drawing view is the active view of
 *       the {@code DrawingEditor}.
 * </ol>
 *
 ...
 * {@code DragTracker} of a {@code SelectionTool}. In turn {@code
 * org.jhotdraw.draw.selectiontool.DragTracker} invokes "track" methods on a {@code Handle} which in
 * turn changes an aspect of a figure.<br>
 * Client: {@link org.jhotdraw.draw.tool.SelectionTool}; Handler: {@link
 * org.jhotdraw.draw.tool.DragTracker}, {@link org.jhotdraw.draw.handle.Handle}.
 *
 * <p><em>Mediator</em><br>
 * {@code DrawingEditor} acts as a mediator for coordinating drawing tools and drawing views:<br>
 * Mediator: {@link DrawingEditor}; Colleagues: {@link DrawingView}, {@link
 ...
 * selected figures.<br>
 * Subject: {@link org.jhotdraw.draw.DrawingView}; Observer: {@link
 * org.jhotdraw.draw.event.FigureSelectionListener}; Event: {@link
 ...
 * org.jhotdraw.draw.event.FigureListener}; Event: {@link org.jhotdraw.draw.event.FigureEvent};
 ...
 * Subject: {@link Handle}; Observer: {@link org.jhotdraw.draw.event.HandleListener}; Event: {@link
 * org.jhotdraw.draw.event.HandleEvent}; Concrete Observer: {@link DrawingView}.
 *
 * <p><em>Strategy</em><br>
...
public interface DrawingView {
  ...
  /**
   * Informs the view that it has been added to the specified editor. The view must draw the tool of
   * the editor, if getActiveView() of the editor returns the view.
   */
  public void addNotify(DrawingEditor editor);

  /**
   * Informs the view that it has been removed from the specified editor. The view must not draw the
   * tool from the editor anymore.
   */
  ...
  /** This is a convenience method for invoking {@code getComponent().requestFocus()}. */
  ...
  /**
   * Gets the current constrainer of this view. If isConstrainerVisible is true, this method returns
   * getVisibleConstrainer, otherwise it returns getInvisibleConstrainer. This is a bound property.
   */
  public Constrainer getConstrainer();

  /**
   * Sets the editor's constrainer for this view, for use, when the visible constrainer is turned
   * on. This is a bound property.
   */
  public void setVisibleConstrainer(Constrainer constrainer);

  /**
   * Gets the editor's constrainer for this view, for use, when the visible constrainer is turned
   * on. This is a bound property.
   */
  ...
  /**
   * Returns true, if the visible Constrainer is in use, returns false, if the invisible Constrainer
   * is in use. This is a bound property.
   */
   ...
   *
   * <p>The drawing framework supports only two use cases, where this component may be accessed:
   *
   * <ul>
   *   <li>The component can be used to add the drawing view to a parent Swing component.
   *   <li>The currently active {@link org.jhotdraw.draw.tool.Tool} may add child components to the
   *       drawing view. The tool <b>must</b> remove these components when it becomes inactive.
   * </ul>
   */
