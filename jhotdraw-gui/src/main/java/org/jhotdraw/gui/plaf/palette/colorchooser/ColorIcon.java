...
public class ColorIcon implements javax.swing.Icon {
  ...
  public ColorIcon(Color color) {
    this(
        color,
        (color == null)
            ? ResourceBundleUtil.getBundle("org.jhotdraw.gui.Labels")
                .getToolTipTextProperty("ColorChooser.colorSwatch.noColor")
  ...
  public void paintIcon(Component c, Graphics g, int x, int y) {
    ...
    if (fillColor == null || fillColor.getAlpha() == 0) {
      if (width == noColorImage.getWidth() && height == noColorImage.getHeight()) {
        ...
        int[] xpoints =
            new int[] {x + 2, x + width - 5, x + width - 3, x + width - 3, x + 4, x + 2};
        int[] ypoints =
            new int[] {y + height - 5, y + 2, y + 2, y + 4, y + height - 3, y + height - 3};
